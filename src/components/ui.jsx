...
export function Eyebrow({ children, className = '' }) {
  return (
    <p className={`text-xs font-black text-orange-500 uppercase tracking-widest ${className}`}>
      {children}
    </p>
  )
}
...
export function SectionHeading({ eyebrow, title, lede, align = 'center', as: As = 'h2' }) {
  const alignCls = align === 'center' ? 'text-center mx-auto' : 'text-left'
  return (
    <div className={`${alignCls} max-w-2xl ${align === 'center' ? 'mb-12' : 'mb-8'}`}>
      {eyebrow && <Eyebrow className="mb-2">{eyebrow}</Eyebrow>}
      <As className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight leading-tight">
        {title}
      </As>
      {lede && <p className="mt-4 text-lg text-gray-500 leading-relaxed">{lede}</p>}
    </div>
  )
}
...
export function Pill({ icon: Icon, children, tone = 'orange' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full border ${tones[tone]}`}>
      {Icon && <Icon size={13} />}
      {children}
    </span>
  )
