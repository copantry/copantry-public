...
import { ArrowRight, Camera, ChefHat, ShoppingCart, RefreshCw } from 'lucide-react'
    ...
    <>
      <Section className="hero-gradient pt-16 pb-12 md:pt-20 md:pb-16" width="max-w-3xl">
        <div className="text-center">
          ...
          <p className="mt-6 text-lg md:text-xl text-gray-600 leading-relaxed">{c.lede}</p>
        </div>
      </Section>
            ...
            return (
              <li key={s.title} className="flex gap-5">
                <div className="flex flex-col items-center shrink-0">
                  ...
                  {i < c.steps.length - 1 && <div className="w-0.5 flex-1 bg-gray-100 mt-2" />}
                </div>
                <div className="pb-2">
                  <p className="text-xs font-black text-orange-500 uppercase tracking-widest mb-1">{c.stepLabel} {i + 1}</p>
        ...
        <Section className="py-16" width="max-w-3xl">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {explore.map(({ label, to }) => (
              ...
              >
                <span className="font-bold text-gray-800 group-hover:text-orange-600">{label}</span>
