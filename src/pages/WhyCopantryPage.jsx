...
const MATRIX_SCORES = {
  'Calendar-first meal planners': ['no', 'no', 'no', 'partial', 'partial', 'partial'],
  'Recipe-savers & recipe boxes': ['no', 'no', 'no', 'no', 'partial', 'partial'],
  'Basic shopping-list apps': ['no', 'no', 'no', 'partial', 'yes', 'partial'],
}

const Cell = ({ score }) => {
  if (score === 'yes') return <Check size={18} className="text-green-600 mx-auto" />
  if (score === 'partial') return <Minus size={18} className="text-amber-500 mx-auto" />
...
export default function WhyCopantryPage() {
  return (
    <>
      <Section className="hero-gradient pt-16 pb-12 md:pt-20 md:pb-16" width="max-w-3xl">
        <div className="text-center">
          ...
          <p className="mt-6 text-lg md:text-xl text-gray-600 leading-relaxed">
            Most kitchen apps do one piece of the job. Calendar-first planners give you an empty
            week. Recipe-savers store links but do not know your kitchen. List apps make a list but
            do not plan or track expiry. Copantry connects all three: it knows what you have, tells
            you what to cook before it expires, and builds your shopping list around the gaps.
          </p>
        </div>
      </Section>
          ...
          {CATEGORIES.map(({ name, fair, gap }) => (
            <div key={name} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-sm text-gray-600 leading-relaxed mb-3">
                <span className="font-semibold text-green-700">Good at: </span>{fair}
              </p>
              <p className="text-sm text-gray-600 leading-relaxed">
                <span className="font-semibold text-rose-600">The gap: </span>{gap}
              </p>
            </div>
          ))}
        </div>
      ...
      <section className="bg-gray-50 border-y border-gray-100">
        <Section className="py-14 md:py-20" width="max-w-4xl">
          <SectionHeading eyebrow="Side by side" title="The whole kitchen loop, in one place" />
          <div className="overflow-x-auto -mx-4 px-4">
            <table className="w-full min-w-[640px] border-separate border-spacing-0">
              <thead>
                <tr>
                  <th className="text-left text-sm font-bold text-gray-500 p-3 w-1/3">Capability</th>
                  <th className="p-3 text-center text-sm font-extrabold text-orange-600 bg-orange-50 rounded-t-xl">Copantry</th>
                  {Object.keys(MATRIX_SCORES).map((c) => (
                    <th key={c} className="p-3 text-center text-xs font-semibold text-gray-500 align-bottom">{c}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {MATRIX.map((cap, row) => (
                  <tr key={cap}>
                    <td className="p-3 text-sm text-gray-700 border-t border-gray-100">{cap}</td>
                    ...
                    {Object.values(MATRIX_SCORES).map((scores, i) => (
                      <td key={i} className="p-3 border-t border-gray-100 text-center">
          ...
          <p className="text-xs text-gray-400 mt-4 flex items-center gap-3 flex-wrap">
            <span className="flex items-center gap-1"><Check size={13} className="text-green-600" /> Built for this</span>
            <span className="flex items-center gap-1"><Minus size={13} className="text-amber-500" /> Sometimes / partly</span>
            <span className="flex items-center gap-1"><X size={12} className="text-gray-300" /> Not really</span>
          </p>
        </Section>
      </section>
      ...
      <Section className="py-14 md:py-20" width="max-w-3xl">
        <div className="bg-white border border-gray-100 rounded-2xl p-7 shadow-sm">
          <h2 className="text-2xl font-extrabold text-gray-900 mb-3">Copantry probably isn’t for you if…</h2>
          <p className="text-gray-600 leading-relaxed">
            …all you want is somewhere to bookmark recipe links, and you never plan meals or shop
            from a list. A simple recipe-saver will be lighter and that is genuinely fine. Copantry
            earns its place when you want to <span className="font-semibold text-gray-900">waste less, plan around what you already have, and shop smarter</span> — that is the job it is built for.
          </p>
        </div>
      </Section>

      <Faq items={FAQS.whyCopantry} title="Best app for the job? Honest answers" eyebrow="Evaluation" />
