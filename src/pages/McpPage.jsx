...
const TOOLS = [
  {
    ...
    description: 'Create, edit, import, and read recipes — by hand or from a URL, text, photo or PDF.',
    tools: [
      {
        name: 'search_recipes',
        ...
        params: [
          { name: 'search', type: 'string?', desc: 'Partial or full recipe title to match (case-insensitive). Omit to list recent recipes.' },
          ...
          { name: 'limit', type: 'number?', desc: 'Maximum results, 1–50. Default 20.' },
        ],
        returns: 'Array of recipe objects with id, title, category, language, servings, prep/cook time, cuisine, and image. Use the id with get_recipe_details.',
        example: '{ "search": "pasta", "category": "MAIN" }',
      },
      {
        name: 'get_recipe_details',
        summary: 'Retrieve the complete recipe: ingredients, step-by-step / workflow instructions, nutrition, and comments.',
        when: 'Use after search_recipes to get full content before planning a meal, editing, or answering detailed cooking questions.',
        params: [
          { name: 'id', type: 'string (UUID)', desc: 'The recipe identifier from search_recipes or get_meal_plan.' },
        ],
        returns: 'Full recipe object with ingredients (quantity, unit, name), notes/steps/workflows, nutrition, and comments.',
        example: '{ "id": "a1b2c3d4-..." }',
      },
      {
        name: 'create_recipe',
        summary: 'Manually create a brand-new recipe from a title, ingredient list and method.',
        when: 'Use when the user dictates or types a recipe directly rather than importing it. Instructions go in notes (free text).',
        params: [
          { name: 'title', type: 'string', desc: 'Recipe title in its original language.' },
          { name: 'ingredients', type: 'array of {name, quantity?, unit?}', desc: 'Full ingredient list. Names are normalised automatically.' },
          { name: 'category', type: 'enum?', desc: 'Meal category. Defaults to MAIN.' },
          { name: 'notes', type: 'string?', desc: 'Cooking method / instructions, free text.' },
          { name: 'servings / prepTime / cookTime', type: 'number?', desc: 'Servings, and prep/cook time in minutes.' },
          { name: 'cuisine / language', type: 'string?', desc: 'Cuisine label and ISO 639-1 language (defaults to en).' },
        ],
        returns: '{ success, recipeId, title } for the created recipe.',
        example: '{ "title": "Aglio e Olio", "ingredients": [{ "name": "spaghetti", "quantity": 400, "unit": "g" }], "notes": "Boil pasta…" }',
      },
      {
        name: 'update_recipe',
        summary: 'Edit an existing recipe’s fields and/or replace its ingredient list.',
        ...
        params: [
          ...
          { name: 'title / notes / category / cuisine', type: 'string/enum?', desc: 'Any fields to change; omitted fields are untouched.' },
          { name: 'servings / prepTime / cookTime', type: 'number?', desc: 'Numeric fields to change.' },
          { name: 'ingredients', type: 'array?', desc: 'If provided, replaces ALL existing ingredients.' },
        ],
      ...
      {
        name: 'delete_recipe',
        summary: 'Permanently delete a recipe. Only the creator or a household OWNER may delete.',
        when: 'Use when the user explicitly wants to remove a recipe. Irreversible — confirm first.',
        params: [{ name: 'id', type: 'string (UUID)', desc: 'Recipe to delete (from search_recipes).' }],
        returns: '{ success: true }.',
        example: '{ "id": "a1b2c3d4-..." }',
      },
      {
        name: 'import_recipe_from_url',
        summary: 'Queue a public URL for AI-powered recipe extraction (background, ~60s).',
        ...
        params: [{ name: 'url', type: 'string (URL)', desc: 'Public recipe URL. Supports blogs, YouTube, Instagram, TikTok, JSON-LD pages.' }],
      ...
      },
      {
        ...
        summary: 'Parse a block of pasted recipe text with AI and save it instantly (synchronous).',
        when: 'Use when the user pastes a full recipe as text. Returns the new recipe id immediately.',
        params: [{ name: 'text', type: 'string', desc: 'Full recipe text including title, ingredients and method (up to ~15000 chars).' }],
        returns: '{ success, recipeId, title }.',
        example: '{ "text": "Pancakes\\n200g flour\\n2 eggs\\n300ml milk\\nMix and fry." }',
      },
      {
        name: 'import_recipe_from_image',
        summary: 'Parse a recipe from a photo (cookbook page, card, screenshot) with vision AI and save it.',
        when: 'Use when the user supplies a recipe photo. Pass the image as base64 (a data: URL is accepted).',
        params: [
          { name: 'base64Image', type: 'string', desc: 'Base64-encoded image bytes (optionally a data: URL).' },
          { name: 'mimeType', type: 'string?', desc: 'e.g. image/jpeg. Defaults to image/jpeg.' },
        ],
        returns: '{ success, recipeId, title }.',
      ...
      },
      {
        ...
        params: [{ name: 'base64Pdf', type: 'string', desc: 'Base64-encoded PDF bytes (optionally a data: URL).' }],
        returns: '{ success, recipeId, title }.',
        example: '{ "base64Pdf": "JVBERi0xLjc…" }',
      },
      {
        name: 'log_recipe_cooked',
        summary: 'Record that a recipe was cooked (adds cook history, marks it tested), with optional guests.',
        ...
        params: [
          { name: 'id', type: 'string (UUID)', desc: 'Recipe that was cooked.' },
          { name: 'occasion', type: 'string?', desc: 'Free-text occasion, e.g. "Sunday dinner".' },
          { name: 'guestNames', type: 'string[]?', desc: 'Guests who ate it (auto-created as household guests).' },
          { name: 'cookedAt', type: 'string? (YYYY-MM-DD)', desc: 'When it was cooked. Defaults to now.' },
        ],
      ...
      },
      {
        ...
        summary: 'Set a recipe to PRIVATE, HOUSEHOLD, BRIGADE (approved followers), or PUBLIC.',
        when: 'Use when the user wants to share or unshare a recipe. Only the creator may change visibility.',
        params: [
          { name: 'id', type: 'string (UUID)', desc: 'Recipe to update.' },
          { name: 'visibility', type: 'enum', desc: 'PRIVATE · HOUSEHOLD · BRIGADE · PUBLIC.' },
        ],
      ...
      },
      {
        ...
        summary: 'AI-estimate and save per-serving nutrition (calories, protein, carbs, fat, fibre).',
        when: 'Use when the user asks for the nutrition/macros of one of their recipes. Results are cached.',
        params: [{ name: 'id', type: 'string (UUID)', desc: 'Recipe to estimate nutrition for.' }],
      ...
      },
      {
        ...
        example: '{ "id": "a1b2c3d4-...", "text": "Add chilli flakes for heat." }',
      },
    ],
  },
  {
    ...
    description: 'Manage the household inventory — add by hand, in bulk, or from text, photos, receipts and PDFs.',
    tools: [
      {
        name: 'list_pantry',
        summary: 'List pantry items, soonest-expiring first. Filter for expired or expiring-soon items.',
        when: 'Use to audit stock, plan waste-free meals, or answer "what do I have?".',
        params: [
          { name: 'expiredOnly', type: 'boolean?', desc: 'Only items already past expiry.' },
          { name: 'expiringInDays', type: 'number?', desc: 'Only items expiring within N days from today.' },
        ],
        returns: 'Array of items with id, name, ingredientId, quantity, unit, expiryDate.',
        example: '{ "expiringInDays": 4 }',
      },
      {
        name: 'add_pantry_item',
        summary: 'Add one ingredient with optional quantity, unit, and expiry date.',
        when: 'Use to log a single ingredient. For many items at once, use add_pantry_items.',
        params: [
          { name: 'name', type: 'string', desc: 'Ingredient name; normalised to singular lowercase automatically.' },
          { name: 'quantity', type: 'number|string?', desc: 'Amount, e.g. 500 or "1/2".' },
          { name: 'unit', type: 'string?', desc: 'e.g. g, ml, l, cup, can.' },
          { name: 'expiryDate', type: 'string? (YYYY-MM-DD)', desc: 'Expiry date. Omit for shelf-stable items.' },
        ],
        returns: '{ success, id, name }.',
        example: '{ "name": "egg", "quantity": 12, "expiryDate": "2025-06-10" }',
      },
      {
        name: 'add_pantry_items',
        summary: 'Add many pantry items in one call (e.g. after a grocery shop).',
        when: 'Prefer this over repeated add_pantry_item calls when logging multiple groceries.',
        params: [{ name: 'items', type: 'array of {name, quantity?, unit?, expiryDate?}', desc: 'Items to add.' }],
        returns: '{ success, created, items }.',
        example: '{ "items": [{ "name": "milk", "quantity": 1, "unit": "l" }, { "name": "flour", "quantity": 1, "unit": "kg" }] }',
      },
      {
        name: 'update_pantry_item',
        summary: 'Update the quantity, unit, or expiry date of an existing pantry item.',
        when: 'Use to correct or adjust an item. Pass expiryDate: null to clear it. Get the id from list_pantry.',
        params: [
          { name: 'id', type: 'string (UUID)', desc: 'Pantry item id (NOT the ingredient id).' },
          { name: 'quantity / unit / expiryDate', type: 'mixed?', desc: 'Fields to change; omitted fields are untouched.' },
        ],
      ...
      },
      {
        ...
        summary: 'Remove a pantry item by id (spoiled, used up, or mis-logged).',
        when: 'Always call list_pantry first to obtain the id — never guess UUIDs.',
        params: [{ name: 'id', type: 'string (UUID)', desc: 'Pantry item id from list_pantry.' }],
        returns: '{ success: true }.',
        example: '{ "id": "e5f6a7b8-..." }',
      },
      {
        name: 'import_pantry_from_text',
        summary: 'Parse a typed grocery list with AI and add every detected item to the pantry.',
        when: 'Use when the user types out what they bought.',
        params: [{ name: 'text', type: 'string', desc: 'Free text listing groceries.' }],
        returns: '{ success, created, items }.',
        example: '{ "text": "2 onions, 500g mince, a dozen eggs, milk" }',
      },
      {
        name: 'import_pantry_from_image',
        summary: 'Parse a fridge/cupboard photo or receipt photo with vision AI and stock the pantry.',
        when: 'Use when the user supplies a photo of groceries. Pass as base64 (a data: URL is accepted).',
        params: [
          { name: 'base64Image', type: 'string', desc: 'Base64-encoded image bytes.' },
      ...
      },
      {
        ...
        summary: 'Parse a PDF receipt or grocery order with AI and stock the pantry.',
        when: 'Use when the user supplies a PDF receipt. Pass as base64 (a data: URL is accepted).',
        params: [{ name: 'base64Pdf', type: 'string', desc: 'Base64-encoded PDF bytes.' }],
        returns: '{ success, created, items }.',
        example: '{ "base64Pdf": "JVBERi0xLjc…" }',
      },
      {
        name: 'estimate_pantry_expiry',
        summary: 'Estimate a best-before window for a food item, optionally saving it to a pantry item.',
        ...
        params: [
          ...
          { name: 'storage', type: 'enum?', desc: 'counter · fridge · freezer.' },
          { name: 'ripeness', type: 'enum?', desc: 'underripe · ripe · very_ripe.' },
          { name: 'cut', type: 'boolean?', desc: 'True if already cut/opened.' },
          { name: 'itemId', type: 'string (UUID)?', desc: 'If set, saves the estimated date onto this pantry item.' },
        ],
        returns: '{ estimate: { minDays, maxDays, tip }, savedExpiryDate }.',
        example: '{ "name": "avocado", "ripeness": "ripe", "storage": "fridge" }',
      },
    ],
  },
  {
    ...
    description: 'View, schedule, adjust, and confirm meals — and annotate the household calendar.',
    tools: [
      {
        name: 'get_meal_plan',
        ...
        params: [
          { name: 'startDate', type: 'string? (YYYY-MM-DD)', desc: 'Inclusive start. Defaults to today.' },
          { name: 'endDate', type: 'string? (YYYY-MM-DD)', desc: 'Inclusive end. Defaults to +7 days.' },
        ],
        returns: '{ entries: [{ id (calendarId), date, mealType, isConfirmed, recipe }], notes }.',
        example: '{ "startDate": "2025-06-02", "endDate": "2025-06-08" }',
      },
      {
        name: 'plan_meal',
        summary: 'Schedule a recipe for lunch or dinner on a date; auto-creates a leftover entry.',
        when: 'Use after the user approves a suggestion. Verify the recipe with search_recipes first.',
        params: [
          { name: 'recipeId', type: 'string (UUID)', desc: 'Recipe to schedule.' },
          { name: 'date', type: 'string (YYYY-MM-DD)', desc: 'Date to schedule.' },
          { name: 'mealType', type: 'enum', desc: 'LUNCH or DINNER.' },
          { name: 'cookFor / partySize', type: 'number?', desc: 'Portions to cook, and how many eat now (rest become leftovers).' },
        ],
      ...
      },
      {
        ...
        summary: 'Change how many portions a planned meal is cooked for; recalculates the leftover.',
        when: 'Use when serving counts change. Get the id from get_meal_plan.',
        params: [
          { name: 'calendarId', type: 'string (UUID)', desc: 'Calendar entry from get_meal_plan.' },
          { name: 'cookFor', type: 'number', desc: 'New portion count.' },
          { name: 'partySize', type: 'number?', desc: 'How many eat now; rest become leftovers.' },
        ],
        returns: '{ success: true }.',
      ...
      },
      {
        ...
        summary: 'Remove a planned meal from the calendar (and its linked leftover).',
        when: 'Use to unschedule a meal. Get the id from get_meal_plan.',
        params: [{ name: 'calendarId', type: 'string (UUID)', desc: 'Calendar entry to remove.' }],
        returns: '{ success: true }.',
        example: '{ "calendarId": "c9d0e1f2-..." }',
      },
      {
        name: 'confirm_meal',
        summary: 'Mark a meal cooked AND deduct its ingredients from the pantry (FIFO by expiry).',
        when: 'Use after the household actually cooked. Provide consumedIngredients to override default quantities.',
        params: [
          { name: 'calendarId', type: 'string (UUID)', desc: 'Calendar entry from get_meal_plan.' },
          { name: 'consumedIngredients', type: 'array? of {ingredientId, quantity}', desc: 'Override consumed quantities. Defaults to the recipe’s amounts.' },
          { name: 'guestNames', type: 'string[]?', desc: 'Guests who ate the meal.' },
        ],
      ...
      {
        name: 'discard_meal',
        summary: 'Mark a meal done WITHOUT deducting any pantry stock (e.g. ate out).',
        when: 'Use when a planned meal happened but should not deduct ingredients.',
        params: [{ name: 'calendarId', type: 'string (UUID)', desc: 'Calendar entry from get_meal_plan.' }],
        returns: '{ success: true }.',
        example: '{ "calendarId": "c9d0e1f2-..." }',
      },
      {
        name: 'set_calendar_note',
        summary: 'Add or replace a free-text note on a meal slot (one per date+mealType).',
        ...
        example: '{ "date": "2025-06-05", "mealType": "DINNER", "summary": "Eating out" }',
      },
    ],
  },
  {
    ...
    description: 'Generate, save, and manage shopping lists from the meal plan or by hand.',
    tools: [
      {
        name: 'get_shopping_lists',
        summary: 'Retrieve all shopping lists with their line items (name, quantity, unit, checked state).',
      ...
      },
      {
        ...
        summary: 'Compute the grocery deficit between planned meals in a date range and current pantry stock.',
        when: 'Use to work out what still needs buying. Returns items only — follow with create_shopping_list to save.',
        params: [
          { name: 'startDate', type: 'string (YYYY-MM-DD)', desc: 'Inclusive start of the meal-plan window.' },
          { name: 'endDate', type: 'string (YYYY-MM-DD)', desc: 'Inclusive end of the meal-plan window.' },
        ],
      ...
      },
      {
        ...
        params: [
          { name: 'name', type: 'string', desc: 'List name, e.g. "Weekly shop".' },
          { name: 'items', type: 'array? of {name, quantity?, unit?}', desc: 'Line items. Defaults to empty.' },
        ],
        returns: '{ success, shoppingListId, itemCount }.',
        example: '{ "name": "Weekly shop", "items": [{ "name": "eggs", "quantity": 12 }] }',
      },
      {
        ...
        params: [
          { name: 'shoppingListId', type: 'string (UUID)', desc: 'List to add to.' },
          { name: 'name', type: 'string', desc: 'Item name.' },
          { name: 'quantity / unit', type: 'mixed?', desc: 'Optional amount and unit.' },
        ],
        returns: '{ success, itemId }.',
        example: '{ "shoppingListId": "f0a1b2c3-...", "name": "butter", "quantity": 250, "unit": "g" }',
      },
      {
        ...
        params: [
          ...
          { name: 'name / quantity / unit', type: 'mixed?', desc: 'Fields to change.' },
          { name: 'isChecked', type: 'boolean?', desc: 'True = bought / ticked off.' },
        ],
      ...
      },
      {
        ...
        params: [{ name: 'id', type: 'string (UUID)', desc: 'Shopping list id.' }],
        returns: '{ success: true }.',
        example: '{ "id": "f0a1b2c3-..." }',
      },
    ],
    ...
    tools: [
      {
        ...
        summary: 'List the user’s recipe collections, including Favorites, with recipe counts.',
        when: 'Use before adding to or referencing a collection.',
        params: [],
        returns: 'Array of { id, name, description, isPublic, isDefault, recipeCount }.',
        example: '{}',
      },
      {
        ...
        params: [
          ...
          { name: 'isPublic', type: 'boolean?', desc: 'Make it publicly shareable. Default false.' },
        ],
      ...
      },
      {
        ...
        params: [
          { name: 'collectionId', type: 'string (UUID)', desc: 'Target collection.' },
          { name: 'recipeId', type: 'string (UUID)', desc: 'Recipe to add.' },
        ],
        returns: '{ success: true }.',
        example: '{ "collectionId": "c0ffee00-...", "recipeId": "a1b2c3d4-..." }',
      },
      {
        ...
        params: [
          { name: 'collectionId', type: 'string (UUID)', desc: 'Collection to edit.' },
          { name: 'recipeId', type: 'string (UUID)', desc: 'Recipe to remove.' },
        ],
        returns: '{ success: true }.',
        example: '{ "collectionId": "c0ffee00-...", "recipeId": "a1b2c3d4-..." }',
      },
      {
        ...
        summary: 'Delete a collection (the Favorites collection cannot be deleted).',
        when: 'Use when the user wants to remove a cookbook entirely.',
        params: [{ name: 'collectionId', type: 'string (UUID)', desc: 'Collection to delete.' }],
        returns: '{ success: true }.',
    ...
    tools: [
      ...
      },
      {
        ...
        params: [
          ...
          { name: 'expiryDate', type: 'string? (YYYY-MM-DD)', desc: 'Use-by date.' },
          { name: 'recipeId', type: 'string (UUID)?', desc: 'Optional link to the source recipe.' },
        ],
        returns: '{ success, leftoverId }.',
        example: '{ "name": "Lasagne", "portions": 3, "expiryDate": "2025-06-07" }',
      },
      {
        ...
        params: [
          ...
          { name: 'portions', type: 'number?', desc: 'New remaining portions.' },
          { name: 'expiryDate', type: 'string?', desc: 'YYYY-MM-DD, or null to clear.' },
        ],
        returns: '{ success: true }.',
      ...
      },
      {
        ...
        params: [{ name: 'id', type: 'string (UUID)', desc: 'Leftover to remove.' }],
        returns: '{ success: true }.',
        example: '{ "id": "1eft0ver-..." }',
      },
    ],
