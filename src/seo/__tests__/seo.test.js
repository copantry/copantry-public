...
import { SHELF_LIFE, learnPath, learnSlug, findByPageSlug } from '../../content/shelfLife.js'
import { POSTS } from '../../content/blog.js'
import {
  LOCALES, HREFLANG_LANGS, HOME, HOW_IT_WORKS, REDUCE_WASTE, HOOK, UI, localizePath,
...
describe('SEO registry', () => {
  ...
  it('every entry has a sensible title and description', () => {
    for (const e of entries) {
      ...
      expect(e.title.length, `title too long: ${e.path}`).toBeLessThanOrEqual(maxTitle)
      expect(e.description, e.path).toBeTruthy()
      expect(e.description.length, `desc too long: ${e.path}`).toBeLessThanOrEqual(200)
    }
  })
  ...
  it('buildHead emits title, canonical and valid JSON-LD for every route', () => {
    for (const e of entries) {
      ...
      const blocks = head.match(/<script type="application\/ld\+json">(.*?)<\/script>/gs) || []
      for (const b of blocks) {
        const json = b.replace(/<\/?script[^>]*>/g, '')
        expect(() => JSON.parse(json), `bad JSON-LD on ${e.path}`).not.toThrow()
      }
    }
  })

  it('escapes HTML-sensitive characters in head output', () => {
    const head = buildHead({ title: 'A & B <x>', description: '"q"', path: '/x' })
  ...
  it('every ingredient has distinct, non-thin guidance (no duplicate pages)', () => {
    for (const item of SHELF_LIFE) {
      for (const key of ['why', 'store', 'cook']) {
        expect(item[key].length, `${item.slug}.${key} thin`).toBeGreaterThan(30)
      }
...
describe('localization (fr/it/es/pt)', () => {
  ...
  it('localized routes are in the registry with lang + full hreflang set', () => {
    const localizedPaths = ['/', '/how-it-works', '/features/reduce-food-waste']
  ...
  it('hreflang hrefs in buildHead point to absolute, correct URLs', () => {
    ...
    expect(head).toContain('rel="alternate" hreflang="fr" href="https://www.copantry.com/fr"')
    expect(head).toContain('hreflang="x-default" href="https://www.copantry.com/"')
  })
})
...
describe('schema builders', () => {
  it('produce the expected @type and shape', () => {
    ...
    const bc = breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'X', path: '/x' }])
