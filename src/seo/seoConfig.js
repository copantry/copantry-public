...
import {
  ...
  howToSchema, breadcrumbSchema, articleSchema,
} from './schema.js'
...
import {
  HOME, HOW_IT_WORKS, REDUCE_WASTE, UI,
  LOCALES, HREFLANG_LANGS, localizePath, pick,
...
const HOW_IT_WORKS_STEPS = [
  { name: 'Tell Copantry what you have', text: 'Add groceries by photo, receipt, voice or text and Copantry builds a live, dated pantry.' },
  { name: 'See what to cook tonight', text: 'Copantry suggests meals built around what is about to expire, scaled to your household.' },
  { name: 'Shop only for the gaps', text: 'It compares your plan to your pantry and builds an aisle-sorted list of only what you are missing.' },
  { name: 'Cook, and it stays in sync', text: 'Mark a meal cooked and Copantry deducts what you used and tracks leftovers.' },
]
...
const STATIC = {
  '/': {
    ...
    description: 'Copantry is a meal planner and pantry tracker that tells you what to cook before food goes to waste and builds your shopping list around what you already have.',
    priority: 1.0, changefreq: 'weekly',
    schema: () => [organizationSchema(), softwareApplicationSchema(), faqSchema(FAQS.home)],
  },
  '/how-it-works': {
    title: titled('How Copantry works — from a full fridge to dinner'),
    description: 'Copantry learns what is in your kitchen, tells you what to cook before it expires, builds a shopping list of only the gaps, and keeps it all in sync as you cook.',
  ...
  '/why-copantry': {
    ...
    description: 'How Copantry compares to calendar-first planners, recipe-savers and basic list apps — it knows what you have, tells you what to cook before it expires, and shops the gaps.',
    priority: 0.9, changefreq: 'monthly',
    schema: () => [
      faqSchema(FAQS.whyCopantry),
      breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Why Copantry', path: '/why-copantry' }]),
    ],
  },
  '/features/ai-assistant-mcp': {
    title: titled('Plan meals from ChatGPT or Claude (MCP)'),
    description: 'Copantry runs an MCP server, so AI assistants like Claude and ChatGPT-style tools can read your pantry, plan meals, import recipes and build shopping lists for you.',
    priority: 0.8, changefreq: 'monthly',
    schema: () => [
      faqSchema(FAQS.aiAssistant),
      breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Features', path: '/how-it-works' }, { name: 'AI assistant (MCP)', path: '/features/ai-assistant-mcp' }]),
    ],
  },
  '/mcp': {
    title: titled('MCP server & API for AI assistants'),
    description: 'Connect Copantry to any MCP-compatible AI assistant. Read your pantry, plan meals, import recipes and manage shopping lists over the Model Context Protocol.',
    priority: 0.6, changefreq: 'monthly',
    schema: () => [faqSchema(FAQS.aiAssistant)],
  },
  '/blog': {
    title: titled('Blog — waste less, cook smarter, spend less'),
    description: 'Practical guides to cooking what you have, planning meals around your fridge, and cutting food waste at home.',
    priority: 0.7, changefreq: 'weekly',
    schema: () => [breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Blog', path: '/blog' }])],
  },
  '/learn': {
    title: titled('How long does food last? Shelf-life guides'),
    description: 'Simple guides to how long everyday food keeps, why it spoils, how to store it so it lasts longer, and what to cook before it goes off.',
    priority: 0.7, changefreq: 'monthly',
    schema: () => [breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Learn', path: '/learn' }])],
  },
  '/about': {
    title: titled('About — helping households waste less food'),
    description: 'Copantry helps households cook what they have before it goes to waste, plan around their kitchen, and shop only for what they are missing.',
  ...
  '/privacy': {
    ...
    description: 'How Copantry collects, uses and protects your personal data, and the rights you have over it.',
...
function featureEntry(slug, page, section, sectionPath) {
  ...
  const blocks = [
    ...
    breadcrumbSchema([
      { name: 'Home', path: '/' },
      { name: section, path: sectionPath === '/features' ? '/how-it-works' : sectionPath },
      { name: page.crumb ?? page.h1, path },
    ]),
  ]
  if (page.howTo) blocks.unshift(howToSchema(page.howTo.title, page.howTo.steps))
  return [path, {
...
const LOCALIZED_SPECS = [
  ...
  { base: '/features/reduce-food-waste', content: REDUCE_WASTE, kind: 'pillar' },
]

/** hreflang alternates for a logical path: one per language + x-default (en). */
function alternatesFor(base) {
  const list = HREFLANG_LANGS.map((l) => ({ hreflang: l, href: `${SITE_URL}${localizePath(base, l)}` }))
...
function localizedTitleDesc(spec, lng) {
  ...
  if (spec.kind === 'home') {
    return { title: `${SITE_NAME} — ${c.h1a} ${c.h1b} ${c.h1accent}`.replace(/\s+/g, ' ').trim(), description: metaDescription(c.heroLede) }
  }
...
function localizedSchema(spec, lng, path) {
  const c = pick(spec.content, lng)
  const crumbsBase = [{ name: (UI[lng] || UI.en).breadcrumbHome, path: localizePath('/', lng) }]
  if (spec.kind === 'home') {
    return [organizationSchema(), softwareApplicationSchema()]
  }
  if (spec.kind === 'how') {
    return [
      howToSchema(c.h1, c.steps.map((s) => ({ name: s.title, text: s.desc }))),
  ...
  return [
    ...
    breadcrumbSchema([...crumbsBase, { name: (UI[lng] || UI.en).breadcrumbFeatures, path: localizePath('/how-it-works', lng) }, { name: c.crumb, path }]),
  ]
}
...
function attachLocalized(reg) {
  for (const spec of LOCALIZED_SPECS) {
    ...
    if (reg[spec.base]) { reg[spec.base].alternates = alternates; reg[spec.base].lang = 'en' }
    ...
    for (const lng of LOCALES) {
      ...
      reg[path] = {
        title, description, lang: lng, alternates,
        priority: spec.kind === 'home' ? 0.9 : 0.7, changefreq: 'monthly',
...
function buildRegistry() {
  ...
  for (const post of POSTS) {
    ...
    reg[path] = {
      ...
      schema: () => [
        articleSchema({ headline: post.title, description: post.description, path, datePublished: post.date }),
        breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Blog', path: '/blog' }, { name: post.category, path }]),
      ],
    }
  }
  ...
  for (const item of SHELF_LIFE) {
    ...
    reg[path] = {
      ...
      schema: () => [
        faqSchema([{ q, a }]),
        breadcrumbSchema([{ name: 'Home', path: '/' }, { name: 'Learn', path: '/learn' }, { name: q, path }]),
      ],
    }
  }
